...
class ClassSpectra(Map):
    ...
    def _init_file_to_class(self):
        ...
        self.found = self.vars.found.__sicdata__
...
#        telescope = telescop
    ...
    def get_circular_region_from_cube(self, coordinates, aperture,
        ...
        pyclass.comm('find')
        self._load_class_variables()
        found = self.vars.found.__sicdata__
        for i in range(found):
            pyclass.comm("get n")
            self._load_class_variables()
            bet_off_0 = float(str(self.vars.off_beta.__sicdata__))
            lambd_off_0 = float(str(self.vars.off_lambda.__sicdata__))
